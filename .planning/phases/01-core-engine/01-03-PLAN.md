...
<task type="auto">
  <name>Task 2: Implement control flow runtime (branching, loops, parallelism) in graph builder</name>
  <files>
    src/configurable_agents/core/control_flow.py
    src/configurable_agents/core/parallel.py
    src/configurable_agents/core/graph_builder.py
...
1. Create `src/configurable_agents/core/control_flow.py`:
   ...
   **create_loop_router(loop_config: LoopConfig, from_node: str) -> Callable:**
   ...
   Note: The loop iteration counter needs to be tracked in state. The graph builder (Task 2 below) will add a hidden `_loop_iteration_{node}` field to the state model for loop edges.

2. Create `src/configurable_agents/core/parallel.py`:
...
3. Update `src/configurable_agents/core/graph_builder.py`:
...
4. Update `src/configurable_agents/core/__init__.py`:
   - Export new functions: `create_routing_function`, `create_loop_router`, `create_fan_out_function`

5. Update `src/configurable_agents/runtime/executor.py`:
   - No changes needed to the execution flow. The graph builder handles the complexity internally. The executor still calls `build_graph()` and `graph.invoke()` the same way.
   - But verify that error handling still covers the new execution paths (conditional routing errors, loop timeouts, parallel failures).
...
Run `pytest tests/ -v --tb=short` -- full test suite passes (zero regressions).
  ...
  <done>
Conditional branching works via route conditions evaluated against state. Loop support repeats nodes with configurable max iterations and exit conditions. Parallel execution fans out via LangGraph Send objects. Existing linear workflows unaffected. All tests pass.
  </done>
</task>

...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/configurable_agents/llm/__init__.py"
      provides: "LLM client for config generation with async streaming"
  ...
  key_links:
    ...
    - from: "src/configurable_agents/ui/gradio_chat.py"
      ...
      via: "LLM async streaming for config generation"
      pattern: "stream_chat|async def.*stream|async_generator"
---
...
<task type="auto">
  ...
  <action>
    ...
    2. **Add LLM streaming support to llm/__init__.py:**
       - Export stream_chat() function using async generator pattern (async def with yield)
       - Use async streaming for non-blocking response generation
       - Support streaming for all providers (OpenAI, Anthropic, Google, Ollama)
       - Each provider uses native streaming API (stream=True parameter, yielding chunks)
    ...
    3. **Create GradioChatUI class in gradio_chat.py:**
       ...
       - generate_config(message: str, history: List[Tuple[str, str]], request: gr.Request) -> Generator[str, None, None]:
         ...
         * Stream LLM response chunk by chunk using async streaming pattern
    ...
    Use async streaming pattern: async generator yielding chunks for non-blocking UI updates
  </action>
  <verify>
  ...
  <done>
    ...
    generate_config() streams LLM responses using async streaming pattern
...
<task type="auto">
  ...
  <action>
    ...
    2. **tests/ui/test_gradio_chat.py:**
       ...
       - test_streaming_chat: Verify async streaming function yields chunks
